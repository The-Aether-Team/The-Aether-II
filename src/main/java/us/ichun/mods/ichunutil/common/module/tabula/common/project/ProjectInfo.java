...
//        model.textureHeight = textureHeight;
		for (CubeGroup cubeGroup : this.cubeGroups)
		{
			this.createGroupCubes(cubeGroup);
		}
		for (CubeInfo cube : this.cubes)
		{
			this.model.cubes.add(cube.createModel(this.model));
		}
	}
	...
	public void destroy()
	{
		if (this.model != null && !this.destroyed)
		{
			...
			{
				TextureUtil.deleteTexture(this.bufferedTextureId);
			}
		}
	...
	public boolean importModel(ModelInfo model, boolean texture)
	{
		for (Map.Entry<String, ModelRenderer> e : model.modelList.entrySet())
		{
			...
			{
				this.textureHeight = (int) rend.textureHeight;
				this.textureWidth = (int) rend.textureWidth;
			}
			...
			else
			{
				...
				{
					ModelBox box = rend.cubeList.get(j);
	...
	public void createChildren(String name, ModelRenderer rend, CubeInfo info)
	{
		...
		for (int i = 0; i < rend.childModels.size(); i++)
		{
			...
			for (int j = 0; j < rend1.cubeList.size(); j++)
			{
				...
				info1.txOffset[0] = ModelRendererAccessor.getTextureOffsetX(rend1);
				info1.txOffset[1] = ModelRendererAccessor.getTextureOffsetY(rend1);

				if (box.boxName != null)
				{
					ModelBase modelBase = ModelRendererAccessor.getBaseModel(rend1);
				...
				TexturedQuad[] quadList = ModelBoxAccessor.getQuadList(box);
	...
	public float getMaximumSize() //horizontal max, vert is halved
	{
		...
		return (float) Math.max((maxX - minX) / this.scale[0], Math.max((maxY - minY) / (this.scale[1] * 2), (maxZ - minZ) / this.scale[2]));
	}

	public ProjectInfo repair()
	{
		while (this.projVersion < ProjectInfo.PROJ_VERSION)
		{
