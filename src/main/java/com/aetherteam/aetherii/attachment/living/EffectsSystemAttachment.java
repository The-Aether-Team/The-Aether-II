...
public class EffectsSystemAttachment {
    ...
    public static final StreamCodec<FriendlyByteBuf, EffectsSystemAttachment> STREAM_CODEC = StreamCodec.composite(
    ...
    public void postTickUpdate(LivingEntity livingEntity) {
            ...
            if (!list.isEmpty()) {
                ...
                if (i != 0) {
                    if (livingEntity.getRandom().nextInt(Math.max(j * i, 1)) == 0) {
                        serverLevel.sendParticles(ColorParticleOption.create(AetherIIParticleTypes.EFFECT_BUILDUP.get(), ARGB.opaque(effect.getType().get().getColor())), livingEntity.getRandomX(0.5), livingEntity.getRandomY(), livingEntity.getRandomZ(0.5), 1, 0, 0, 0, 1.0);
                    }
                }
            }
        }
        ...
        if (this.needSync) {
            this.needSync = false;
            AetherIIDataAttachments.sync(livingEntity, AetherIIDataAttachments.EFFECTS_SYSTEM);
        }
    ...
    public void addBuildup(LivingEntity target, @Nullable Entity directSource, @Nullable Entity source, EffectBuildupPresets.Preset buildup, int amount) {
        ...
        if (!target.hasEffect(effect.value())) {
            double modifiedAmount = amount;
            for (Map.Entry<Attribute, AttributeInstance> attributeEntries : target.getAttributes().attributes.entrySet()) {
                if (attributeEntries.getKey() instanceof EffectResistanceAttribute effectResistanceAttribute && effectResistanceAttribute.matches(effect)) {
