...
public record Subtractive(List<ResourceLocation> textures, Map<String, ResourceLocation> overlays) implements SpriteSource {
    public static final Codec<Subtractive> CODEC = RecordCodecBuilder.create(instance -> instance.group(
    ...
    public SpriteSourceType type() {
        return AetherIISpriteSourceTypes.SUBTRACTIVE;
    }
            ...
            try {
                ...
                for (int y = 0; y < nativeImage.getHeight(); y++) {
                    for (int x = 0; x < nativeImage.getWidth(); x++) {
                        int color = nativeOverlayImage.getLuminanceOrAlpha(x, y) == 0 ? nativeBaseImage.getPixelRGBA(x, y) : 0x00000000;
                ...
                AetherII.LOGGER.error("Unable to create subtractive sprite at {}", this.outputLocation(), exception);
            } finally {
