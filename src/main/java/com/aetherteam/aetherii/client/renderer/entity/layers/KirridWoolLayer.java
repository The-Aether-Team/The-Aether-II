...
public class KirridWoolLayer extends RenderLayer<Kirrid, EntityModel<Kirrid>> {
    private static final ResourceLocation ARCTIC_KIRRID_WOOL_TEXTURE = new ResourceLocation(AetherII.MODID, "textures/entity/mobs/kirrid/arctic/kirrid_arctic_wool.png");
    private static final ResourceLocation ARCTIC_KIRRID_BABY_WOOL_TEXTURE = new ResourceLocation(AetherII.MODID, "textures/entity/mobs/kirrid/arctic/kirrid_arctic_baby_wool.png");
    private static final ResourceLocation HIGHFIELDS_KIRRID_WOOL_TEXTURE = new ResourceLocation(AetherII.MODID, "textures/entity/mobs/kirrid/highfields/kirrid_highfields_wool.png");
    private static final ResourceLocation HIGHFIELDS_KIRRID_BABY_WOOL_TEXTURE = new ResourceLocation(AetherII.MODID, "textures/entity/mobs/kirrid/highfields/kirrid_highfields_baby_wool.png");
    private static final ResourceLocation MAGNETIC_KIRRID_WOOL_TEXTURE = new ResourceLocation(AetherII.MODID, "textures/entity/mobs/kirrid/magnetic/kirrid_magnetic_wool.png");
    private static final ResourceLocation MAGNETIC_KIRRID_BABY_WOOL_TEXTURE = new ResourceLocation(AetherII.MODID, "textures/entity/mobs/kirrid/magnetic/kirrid_magnetic_baby_wool.png");
    ...
    public void render(PoseStack poseStack, MultiBufferSource bufferSource, int packedLight, Kirrid kirrid, float limbSwing, float limbSwingAmount, float partialTick, float ageInTicks, float netHeadYaw, float headPitch) {
        if (kirrid.isInvisible()) {
            ...
            if (flag) {
                ...
                this.getParentModel().renderToBuffer(poseStack, consumer, packedLight, LivingEntityRenderer.getOverlayCoords(kirrid, 0.0F), 0.0F, 0.0F, 0.0F, 1.0F);
            }
        } else if (!kirrid.isSheared()) {
            int color;
            if (kirrid.hasCustomName() && kirrid.getName().getString().equals("jeb_")) {
                ...
                coloredCutoutModelCopyLayerRender(this.getParentModel(), this.getParentModel(), this.getTexture(kirrid), poseStack, bufferSource, packedLight, kirrid, limbSwing, limbSwingAmount, ageInTicks, netHeadYaw, headPitch, partialTick, red(color), green(color), blue(color));
            } else if (kirrid.getColor().isPresent()) {
                color = Kirrid.getDecimalColor(kirrid.getColor().get());
                coloredCutoutModelCopyLayerRender(this.getParentModel(), this.getParentModel(), this.getTexture(kirrid), poseStack, bufferSource, packedLight, kirrid, limbSwing, limbSwingAmount, ageInTicks, netHeadYaw, headPitch, partialTick, red(color), green(color), blue(color));
            }
        }
    }
