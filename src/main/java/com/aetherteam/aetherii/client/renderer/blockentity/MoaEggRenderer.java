...
public class MoaEggRenderer implements BlockEntityRenderer<MoaEggBlockEntity> {
    ...
    public void render(MoaEggBlockEntity blockEntity, float partialTick, PoseStack poseStack, MultiBufferSource buffer, int packedLight, int packedOverlay, Vec3 cameraPos) {
        if (blockEntity.getBlockState().getBlock() instanceof MoaEggBlock) {
            ...
            TextureAtlasSprite feathersSprite = this.moaFeathersAtlas.getSprite(Identifier.fromNamespaceAndPath(AetherII.MODID, "entity/moa_egg/moa_egg_feather_" + blockEntity.getBlockState().getValue(MoaEggBlock.FEATHER_SHAPE).getSerializedName() + "_" + blockEntity.getBlockState().getValue(MoaEggBlock.FEATHERS).getSerializedName()));
            ...
            TextureAtlasSprite eyesSprite = this.moaEyesAtlas.getSprite(Identifier.fromNamespaceAndPath(AetherII.MODID, "entity/moa_egg/moa_egg_eyes_" + blockEntity.getBlockState().getValue(MoaEggBlock.EYES).getSerializedName()));
            ...
            TextureAtlasSprite keratinSprite = this.moaKeratinAtlas.getSprite(Identifier.fromNamespaceAndPath(AetherII.MODID, "entity/moa_egg/moa_egg_keratin_" + blockEntity.getBlockState().getValue(MoaEggBlock.KERATIN).getSerializedName()));
