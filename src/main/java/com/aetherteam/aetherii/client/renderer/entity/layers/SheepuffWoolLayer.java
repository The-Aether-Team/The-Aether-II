...
public class SheepuffWoolLayer extends RenderLayer<Sheepuff, SheepuffModel<Sheepuff>> {
    private static final ResourceLocation SHEEPUFF_WOOL_TEXTURE = new ResourceLocation(AetherII.MODID, "textures/entity/mobs/sheepuff/sheepuff_wool.png");
    ...
    public void render(PoseStack poseStack, MultiBufferSource buffer, int packedLight, Sheepuff sheepuff, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch) {
        if (sheepuff.isInvisible()) {
            ...
            if (sheepuff.hasCustomName() && sheepuff.getName().getString().equals("jeb_")) {
                ...
                color = Sheepuff.getDecimalColor(sheepuff.getColor());
            }
            coloredCutoutModelCopyLayerRender(this.getParentModel(), this.getParentModel(), SHEEPUFF_WOOL_TEXTURE, poseStack, buffer, packedLight, sheepuff, limbSwing, limbSwingAmount, ageInTicks, netHeadYaw, headPitch, partialTicks, red(color), green(color), blue(color));
        }
    }
