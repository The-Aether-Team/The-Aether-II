...
public record BetterCooldown() {
    ...
    public float get(ItemStack itemStack, @Nullable ClientLevel clientLevel, @Nullable LivingEntity owner, int i) {
        if (owner instanceof Player player) {
            return player.getCooldowns().getCooldownPercent(itemStack.getItem(), 0.0F);
        } else if (owner instanceof CooldownEntity entity) {
            return entity.getCooldowns().getCooldownPercent(itemStack.getItem(), 0.0F);
        }
        return 0.0F;
    }
