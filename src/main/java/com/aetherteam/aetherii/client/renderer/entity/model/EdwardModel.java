...
public class EdwardModel extends EntityModel<EdwardRenderState> implements ArmedModel<EdwardRenderState>, HeadedModel {
    ...
    @Override
    public void setupAnim(EdwardRenderState state) {
        ...
        if (state.isSitting) {
            ...
            this.armright1.xRot = Mth.cos(f1 * 0.6662F + (float) Math.PI) * 2.0F * f2 * 0.5F / f;
            this.armleft1.xRot = Mth.cos(f1 * 0.6662F) * 2.0F * f2 * 0.5F / f;
            this.armright1.zRot = 0.0F;
            this.armleft1.zRot = 0.0F;
            this.legright1.xRot = Mth.cos(f1 * 0.6662F) * 1.4F * f2 / f;
            this.legleft1.xRot = Mth.cos(f1 * 0.6662F + (float) Math.PI) * 1.4F * f2 / f;
            ...
            AnimationUtils.bobModelPart(this.armright1, state.ageInTicks, -1.0F);
            AnimationUtils.bobModelPart(this.armleft1, state.ageInTicks, 1.0F);
        }
    }
