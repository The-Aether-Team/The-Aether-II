...
public class ImmolationEffect extends MobEffect {
    ...
    public void applyEffectTick(LivingEntity livingEntity, int amplifier) {
        if (livingEntity.level() instanceof ServerLevel serverLevel) {
            for (Entity entity : serverLevel.getEntities(livingEntity, AABB.ofSize(livingEntity.position(), 5, 5, 5), (entity) -> entity instanceof LivingEntity living && !living.hasEffect(AetherIIMobEffects.IMMOLATION.get()))) {
                if (entity instanceof LivingEntity living) {
                    AetherIIDataAttachments.get(living, AetherIIDataAttachments.EFFECTS_SYSTEM).addBuildup(living, EffectBuildupPresets.IMMOLATION, 20);
                }
            }
        ...
        if (livingEntity.isInWater()) {
            livingEntity.removeEffect(AetherIIMobEffects.IMMOLATION.get());
        }
    }
    ...
    public static void onEntityPostTick(LivingEvent.LivingTickEvent event) {
        ...
        if (attachment.hasBuildup(effect(AetherIIMobEffects.IMMOLATION))) {
            if (livingEntity.isInWater()) {
                attachment.removeBuildup(effect(AetherIIMobEffects.IMMOLATION));
            }
        }
    }
}
