...
public class GelDropsFunction extends LootItemConditionalFunction {
    ...
    private GelDropsFunction(LootItemCondition[] conditions, NumberProvider value) {
        super(conditions);
        this.value = value;
    }
    ...
    public LootItemFunctionType getType() {
        return AetherIILootFunctions.GEL_DROPS.get();
    }

    public static class Serializer extends LootItemConditionalFunction.Serializer<GelDropsFunction> {
        @Override
        public void serialize(JsonObject jsonObject, GelDropsFunction function, JsonSerializationContext context) {
            super.serialize(jsonObject, function, context);
            jsonObject.add("count", context.serialize(function.value));
        }

        @Override
        public GelDropsFunction deserialize(JsonObject jsonObject, JsonDeserializationContext context, LootItemCondition[] conditions) {
            NumberProvider count = GsonHelper.getAsObject(jsonObject, "count", context, NumberProvider.class);
            return new GelDropsFunction(conditions, count);
        }
    }
}
