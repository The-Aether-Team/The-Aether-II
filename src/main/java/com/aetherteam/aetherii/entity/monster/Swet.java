...
public class Swet extends Monster {
    ...
    protected void registerGoals() {
        ...
        this.targetSelector.addGoal(1, new NearestAttackableTargetGoal<>(this, Player.class, true, target -> target instanceof Player player && this.canLatch(player)));
    }
    ...
    protected void defineSynchedData() {
        ...
        this.entityData.define(DATA_WATER_DAMAGE_ID, 0.0F);
        this.entityData.define(DATA_FOOD_SATURATION_ID, 1.0F);
        this.entityData.define(DATA_SWET_SCALE_ID, 0.95F);
    }

    public static boolean checkSwetSpawnRules(EntityType<Swet> entityType, ServerLevelAccessor level, MobSpawnType reason, BlockPos pos, RandomSource random) {
    ...
    public boolean canLatch(Player player) {
        return !player.isInWater()
                ...
                && AetherIIDataAttachments.get(player, AetherIIDataAttachments.SWET_LATCH).canLatchOn();
    }
    ...
    public void readAdditionalSaveData(CompoundTag input) {
        ...
        this.setWaterDamage(input.getFloat("WaterDamageScale"));
        this.setSwetScale(input.contains("SwetScale") ? input.getFloat("SwetScale") : 0.95F);
        this.setFoodSaturation(input.getFloat("Saturation"));
    }
                ...
                if (target != null) {
                    ...
                    if (this.swet.getBoundingBox().intersects(target.getBoundingBox())) {
                        if (target instanceof Player player) {
                            AetherIIDataAttachments.get(player, AetherIIDataAttachments.SWET_LATCH).latchSwet(this.swet);
                        }
                    }
                }
            }
    ...
    public record Overlay(ResourceLocation left1, ResourceLocation left2, ResourceLocation right1, ResourceLocation right2) {
        public static Overlay create(String name) {
            return new Overlay(
                    new ResourceLocation(AetherII.MODID, "overlay/swet/" + name + "_left_1"),
                    new ResourceLocation(AetherII.MODID, "overlay/swet/" + name + "_left_2"),
                    new ResourceLocation(AetherII.MODID, "overlay/swet/" + name + "_right_1"),
                    new ResourceLocation(AetherII.MODID, "overlay/swet/" + name + "_right_2"));
        }
    }
}
