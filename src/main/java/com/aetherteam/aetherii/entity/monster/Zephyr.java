...
public class Zephyr extends Mob implements Enemy {
    ...
    protected void defineSynchedData() {
        ...
        this.entityData.define(DATA_BLOW_CHARGE_ID, -40);
    }
    ...
    public void travel(Vec3 p_415638_) {
        if (this.isControlledByLocalInstance()) {
            if (this.isInWater()) {
                ...
                float groundFactor = 0.16277137F / (friction * friction * friction);
                friction = 0.91F;
                if (this.onGround()) {
                    friction = this.level().getBlockState(this.getBlockPosBelowThatAffectsMyMovement()).getBlock().getFriction() * 0.91F;
                }

                this.moveRelative(this.onGround() ? 0.1F * groundFactor : 0.02F, p_415638_);
                this.move(MoverType.SELF, this.getDeltaMovement());
        ...
        if (this.getBlowChargeTime() >= 25 && this.getBlowChargeTime() < 50) {
            ...
            list.forEach(entity -> {
                if (entity instanceof LivingEntity livingEntity) {
                    ...
                    } else if (livingEntity.hasEffect(AetherIIMobEffects.WEBBED.get())) {
    ...
    protected static class ZephyrShootSnowballGoal extends Goal {
        ...
        @Override
        public boolean canUse() {
            return this.zephyr.getTarget() != null && this.zephyr.getTarget().isAlive() && this.zephyr.distanceToSqr(this.zephyr.getTarget()) >= this.attackThresholdSqr && this.zephyr.distanceToSqr(this.zephyr.getTarget()) < this.attackFarLimitSqr && this.zephyr.getBlowChargeTime() == -40 && !this.zephyr.getTarget().hasEffect(AetherIIMobEffects.WEBBED.get());
        }
            ...
            if (this.trackedTarget != null) {
                ...
                if (this.zephyr.getProjectileChargeTime() == 10) {
                    ...
                    if (!this.zephyr.hasLineOfSight(this.trackedTarget)
                            ...
                            || this.zephyr.getTarget().hasEffect(AetherIIMobEffects.WEBBED.get())) {
                        this.trackedTarget = null;
                    }
                }
            }
    ...
    protected static class RandomFloatAroundGoal extends Goal {
        ...
        public void start() {
            ...
            if (target == null) {
                ...
                this.zephyr.getMoveControl().setWantedPosition(d0, d1, d2, 1.0);
            } else if ((this.zephyr.getProjectileChargeTime() == -40 && this.zephyr.getRandom().nextInt(6) != 0) || target.hasEffect(AetherIIMobEffects.WEBBED.get())) {
            ...
            } else if (this.zephyr.getBlowChargeTime() == -40 && !target.hasEffect(AetherIIMobEffects.WEBBED.get())) {
