...
public class TempestThunderball extends AbstractHurtingProjectile {
    ...
    public TempestThunderball(Level level, LivingEntity shooter, double accelX, double accelY, double accelZ) {
        super(AetherIIEntityTypes.TEMPEST_THUNDERBALL.get(), shooter, accelX, accelY, accelZ, level);
        this.setNoGravity(true);
    }
    ...
    private void spawnElectricCircle(double x, double z) {
        ...
        electricCircle.addEffect(new MobEffectInstance(AetherIIMobEffects.ELECTRIC_SHOCK.get(), 1, 0, false, false, false));
            ...
            if (entity instanceof LivingEntity livingEntity) {
                if (livingEntity.level() instanceof ServerLevel serverLevel) {
                    if (this.getOwner() instanceof LivingEntity livingOwner) {
                        livingEntity.hurt(AetherIIDamageTypes.indirectEntityDamageSource(livingEntity.level(), AetherIIDamageTypes.SHOCK, this, livingOwner), 3.0F);
                    } else {
                        livingEntity.hurt(AetherIIDamageTypes.damageSource(livingEntity.level(), AetherIIDamageTypes.SHOCK), 3.0F);
                    }
                }
            }
        }
