...
public class ThrownPrismallardEgg extends ThrowableItemProjectile {
    ...
    public ThrownPrismallardEgg(Level level, LivingEntity mob, ItemStack itemStack) {
        super(AetherIIEntityTypes.PRISMALLARD_EGG.get(), mob, level);
        this.setItem(itemStack);
    }

    public ThrownPrismallardEgg(Level level, double x, double y, double z, ItemStack itemStack) {
        super(AetherIIEntityTypes.PRISMALLARD_EGG.get(), x, y, z, level);
        this.setItem(itemStack);
    }
        ...
        if (!this.level().isClientSide()) {
            if (this.random.nextInt(8) == 0) {
                ...
                for(int i = 0; i < count; ++i) {
                    Prismallard prismallard = AetherIIEntityTypes.PRISMALLARD.get().create(this.level());
                    if (prismallard != null) {
                        prismallard.setAge(-24000);
                        prismallard.moveTo(this.getX(), this.getY(), this.getZ(), this.getYRot(), 0.0F);
