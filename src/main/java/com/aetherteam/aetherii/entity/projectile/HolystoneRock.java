...
public class HolystoneRock extends ThrowableItemProjectile {
    ...
    protected Item getDefaultItem() {
        return AetherIIBlocks.HOLYSTONE_ROCK.get().asItem();
    }

    @Override
    public void handleEntityEvent(byte id) {
        if (id == 3) {
            ParticleOptions particleoptions = new ItemParticleOption(ParticleTypes.ITEM, new ItemStack(AetherIIBlocks.HOLYSTONE_ROCK.get().asItem()));
            for (int i = 0; i < 8; ++i) {
