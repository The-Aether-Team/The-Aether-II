...
public class EatAetherGrassGoal extends Goal {
    ...
    private static final Predicate<BlockState> IS_TALL_GRASS = (state) -> state.is(AetherIIBlocks.SHORT_AETHER_GRASS.get())
            || state.is(AetherIIBlocks.MEDIUM_AETHER_GRASS.get())
            || state.is(AetherIIBlocks.TALL_AETHER_GRASS.get())
            || state.is(AetherIIBlocks.AETHER_FERN.get())
            || state.is(AetherIIBlocks.SHIELD_FERN.get());
    ...
    public void tick() {
        ...
        if (this.eatAnimationTick == this.adjustedTickDelay(4)) {
            BlockPos blockPos = this.mob.blockPosition();
            if (IS_TALL_GRASS.test(this.mob.level().getBlockState(blockPos))) {
                if (ForgeEventFactory.getMobGriefingEvent((ServerLevel) this.mob.level(), this.mob)) {
