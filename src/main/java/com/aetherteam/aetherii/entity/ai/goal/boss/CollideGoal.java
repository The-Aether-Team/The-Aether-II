    ...
    public void tick() {
        ...
        if (this.slider.level() instanceof ServerLevel serverLevel) {
            for (Entity entity : this.slider.level().getEntities(this.slider, collisionBounds)) {
                ...
                if (entity instanceof LivingEntity livingEntity && !livingEntity.isInvulnerableTo(damageSource) && entity.hurt(damageSource, 6.0F)) {
                    if (!livingEntity.isBlocking()) {
                        AetherIIDataAttachments.get(livingEntity, AetherIIDataAttachments.EFFECTS_SYSTEM).addBuildup(livingEntity, this.slider, EffectBuildupPresets.STUN, 150);
                    }
