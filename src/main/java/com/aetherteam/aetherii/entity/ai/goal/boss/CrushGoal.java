...
public class CrushGoal extends Goal {
    ...
    public void start() {
        ...
        if (this.slider.level() instanceof ServerLevel serverLevel) {
            if (ForgeEventFactory.getMobGriefingEvent(serverLevel, this.slider)) {
                if (this.slider.getMoveDirection() != null) {
                    AABB crushBox = this.slider.getBoundingBox().expandTowards(Vec3.atLowerCornerOf(this.slider.getMoveDirection().getNormal()).scale(0.1));
        ...
        if (crushed) {
            this.slider.playSound(SoundEvents.GENERIC_EXPLODE, 3.0F, (0.625F + (this.slider.getRandom().nextFloat() - this.slider.getRandom().nextFloat()) * 0.2F) * 0.7F);
    ...
    private boolean blocksBetween(Slider slider) {
        ...
        BlockPos sliderPos = slider.blockPosition();
        for (BlockPos pos : BlockPos.betweenClosed(
                Math.min(targetPos.getX(), sliderPos.getX()),
                Math.min(targetPos.getY(), sliderPos.getY()),
                Math.min(targetPos.getZ(), sliderPos.getZ()),
                Math.max(targetPos.getX(), sliderPos.getX()),
                Math.max(targetPos.getY(), sliderPos.getY()),
                Math.max(targetPos.getZ(), sliderPos.getZ()))) {
