...
public class Aerwhale extends PathfinderMob {
    ...
    public void travel(Vec3 vector) {
        if (this.isControlledByLocalInstance()) {
            ...
            this.moveRelative(speed, vector);
    ...
    public static class RandomFloatAroundGoal extends Goal {
        ...
        private static Vec3 chooseRandomPosition(Level level, Vec3 pos, RandomSource random) {
            ...
            double d1 = Mth.clamp(pos.y + (random.nextFloat() * 2.0F - 1.0F) * 16.0F, level.getMinBuildHeight(), level.getMaxBuildHeight());
        ...
        private static Vec3 chooseRandomPositionWithRestriction(Mob mob, Vec3 pos, RandomSource random) {
            ...
            return mob.hasRestriction() && !mob.getRestrictCenter().closerToCenterThan(vec3, mob.getRestrictRadius()) ? null : vec3;
        }
    }
    ...
    public static class AerwhaleMoveControl extends MoveControl {
        ...
        private boolean canReach(Vec3 delta) {
            ...
            if (this.careful) {
                AABB inflated = aabb1.inflate(1.0);
                for (BlockPos blockpos : BlockPos.betweenClosed(
                    Mth.floor(inflated.minX), Mth.floor(inflated.minY), Mth.floor(inflated.minZ),
                    Mth.floor(inflated.maxX), Mth.floor(inflated.maxY), Mth.floor(inflated.maxZ)
                )) {
            ...
            for (BlockPos pos : BlockPos.betweenClosed(
                ...
                if (!this.blockTraversalPossible(this.whale.level(), vec3, vec31, pos, flag1, flag)) {
                    return false;
                }
            }
            return true;
        ...
        private boolean blockTraversalPossible(
            ...
            if (blockstate.isAir()) {
                ...
                boolean flag1 = blockstate.getCollisionShape(level, pos).isEmpty();
                if (!this.careful) {
                    return flag1;
                } else {
