...
public class AetherPortalShape {
    private static final BlockBehaviour.StatePredicate FRAME = AetherPortalShape::isFrame;
    ...
    private int getDistanceUntilTop(BlockPos.MutableBlockPos mutablePos) {
        for (int i = 0; i < 21; ++i) {
            ...
            for (int j = 0; j < this.width; ++j) {
                ...
                if (blockState.is(AetherIIBlocks.AETHER_PORTAL.get())) {
    ...
    private static boolean isEmpty(BlockState state) {
        return state.isAir() || state.is(Blocks.WATER) || state.is(AetherIIBlocks.AETHER_PORTAL.get());
    }

    private static boolean isFrame(BlockState state, BlockGetter level, BlockPos pos) {
        return AetherIIConfig.COMMON.use_quartz_aether_portal_frame.get() ? isQuartzFrame(state) : state.is(Blocks.GLOWSTONE);
    }

    private static boolean isQuartzFrame(BlockState state) {
