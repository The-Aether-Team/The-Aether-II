...
public abstract class AetherIIBlockStateProvider extends NitrogenBlockStateProvider {
    ...
    public ResourceLocation texture(String name, String location, String suffix) {
        return this.extend(this.texture(name), suffix);
    }

    @Override
    public void portal(Block block) {
        ResourceLocation texture = this.modLoc("block/" + this.name(block));
        ModelFile northSouth = this.models().withExistingParent(this.name(block) + "_ns", this.mcLoc("block/nether_portal_ns"))
                .texture("particle", texture)
                .texture("portal", texture);
        ModelFile eastWest = this.models().withExistingParent(this.name(block) + "_ew", this.mcLoc("block/nether_portal_ew"))
                .texture("particle", texture)
                .texture("portal", texture);
        this.getVariantBuilder(block).forAllStates((state) -> ConfiguredModel.builder()
                .modelFile(state.getValue(BlockStateProperties.HORIZONTAL_AXIS) == Direction.Axis.X ? northSouth : eastWest)
                .build());
    }

    public void block(Block block, Block textureBlock) {
        this.simpleBlock(block, this.models().cubeAll(this.name(block), this.texture(this.name(textureBlock))));
    }
    ...
    public ModelFile grassBlockTinted(Block block, Block dirtBlock) {
        return this.models().withExistingParent(this.name(block), this.mcLoc("block/block")).renderType(new ResourceLocation("cutout"))
                .texture("particle", this.texture(this.name(dirtBlock)))
                .texture("bottom", this.texture(this.name(dirtBlock)))
                .texture("top_1", this.texture(this.name(block) + "_top_1"))
                .texture("top_2", this.texture(this.name(block) + "_top_2"))
                .texture("top_3", this.texture(this.name(block) + "_top_3"))
                .texture("side", this.texture(this.name(block) + "_side"))
                .texture("overlay_1", this.texture(this.name(block) + "_side_overlay_1"))
                .texture("overlay_2", this.texture(this.name(block) + "_side_overlay_2"))
                .texture("overlay_3", this.texture(this.name(block) + "_side_overlay_3"))
    ...
    public void dirtPath(Block block, Block dirtBlock) {
        ModelFile path = this.models().withExistingParent(this.name(block), this.mcLoc("block/dirt_path"))
                .texture("particle", this.texture(this.name(dirtBlock)))
                .texture("top", this.texture(this.name(block) + "_top"))
                .texture("side", this.texture(this.name(block) + "_side"))
                .texture("bottom", this.texture(this.name(dirtBlock)));
    ...
    public void farmland(Block block, Block dirtBlock) {
        ModelFile farmland = this.models().withExistingParent(this.name(block), this.mcLoc("block/template_farmland"))
                .texture("dirt", this.texture(this.name(dirtBlock)))
                .texture("top", this.texture(this.name(block)));
        ModelFile moist = this.models().withExistingParent(this.name(block) + "_moist", mcLoc("block/template_farmland"))
                .texture("dirt", this.texture(this.name(dirtBlock)))
                .texture("top", this.texture(this.name(block) + "_moist"));
        this.getVariantBuilder(block).forAllStatesExcept(state -> {
            int moisture = state.getValue(BlockStateProperties.MOISTURE);
            return ConfiguredModel.builder()
                    .modelFile(moisture < 7 ? farmland : moist)
                    .build();
        });
    }

    public void snowLayer(Block block, Block base) {
        ResourceLocation texture = this.texture(this.name(base));
        this.getVariantBuilder(block).forAllStatesExcept((state) -> {
    ...
    public void mossyWisproot(RotatedPillarBlock block, RotatedPillarBlock baseBlock) {
        ResourceLocation side = this.texture(this.name(block), "natural/");
        ResourceLocation bottom = this.extend(this.texture(this.name(block), "natural/"), "_top");
        ResourceLocation top = this.extend(this.texture(this.name(baseBlock), "natural/"), "_top");
        ModelFile vertical = this.models().cubeColumn(this.name(block), side, bottom).texture("up", top);
    ...
    public void leaves(Block block) {
        this.getVariantBuilder(block).forAllStates((state) -> {
            String texture = this.name(block);
            String modelName = texture;
            AetherIIBlockStateProperties.Mossy mossy = state.getValue(AetherLeavesBlock.MOSSY);
            if (state.getValue(AetherLeavesBlock.SNOWY)) {
                texture += "_snowy";
                modelName += "_snowy";
            } else if (mossy != AetherIIBlockStateProperties.Mossy.NONE) {
                texture += "_" + mossy.getSerializedName();
                modelName += "_" + mossy.getSerializedName();
            }
            ModelFile model = this.models().cubeAll(modelName, this.texture(texture)).renderType(new ResourceLocation("cutout"));
            return ConfiguredModel.builder().modelFile(model).build();
        });
    }
    ...
    public void pottedPlant(Block block, Block flower, String location) {
        ModelFile pot = this.models().withExistingParent(this.name(block), this.mcLoc("block/flower_pot_cross"))
                .texture("plant", this.texture(this.name(flower)))
                .renderType(new ResourceLocation("cutout"));
    ...
    public void shortGrass(Block block) {
        this.getVariantBuilder(block).forAllStates((state) -> {
            boolean snowy = state.getValue(AetherTallGrassBlock.TYPE) == AetherTallGrassBlock.GrassType.SNOWY;
            ModelFile grass;
            if (snowy) {
                grass = this.models().cross(this.name(block) + "_snowy", this.texture(this.name(block) + "_snowy")).renderType(new ResourceLocation("cutout"));
            } else {
    ...
    public void frostedCross(Block block) {
        this.getVariantBuilder(block).forAllStates((state) -> {
            boolean snowy = state.hasProperty(AetherFlowerBlock.SNOWY) && state.getValue(AetherFlowerBlock.SNOWY)
                    || state.hasProperty(AetherTallGrassBlock.TYPE) && state.getValue(AetherTallGrassBlock.TYPE) == AetherTallGrassBlock.GrassType.SNOWY;
            String suffix = snowy ? "_snowy" : "";
            ModelFile grass = this.models().cross(this.name(block) + suffix, this.texture(this.name(block) + suffix)).renderType(new ResourceLocation("cutout"));
            return ConfiguredModel.builder().modelFile(grass).build();
        });
    }
    ...
    public void pottedBush(Block bush, String stem, String location) {
        ModelFile pot = this.pottedStemModel(bush, stem, location)
                .texture("stem", this.texture(stem)).texture("bush", this.texture(this.name(bush)))
    ...
    public BlockModelBuilder pottedStemModel(Block block, String stem, String location) {
        return models().withExistingParent(this.name(block), this.mcLoc("block/block"))
                .texture("particle", this.mcLoc("block/flower_pot")).texture("stem", this.texture(stem)).texture("dirt", this.mcLoc("block/dirt")).texture("flowerpot", this.mcLoc("block/flower_pot"))
    ...
    public void pottedOrangeTree(Block block, Block tree) {
        ModelFile pot = this.models().withExistingParent(this.name(block), this.mcLoc("block/flower_pot_cross")).texture("plant", this.texture(this.name(tree) + "_bottom_0")).renderType(new ResourceLocation("cutout"));
        this.getVariantBuilder(block).partialState().addModels(new ConfiguredModel(pot));
    }
    ...
    private BlockModelBuilder door(DoorBlock block, String name, String model) {
        String halfSuffix = model.startsWith("door_top") ? "_top" : "_bottom";
        return this.models().withExistingParent(name, this.modLoc("block/" + model))
                .texture("front", this.texture(this.name(block) + halfSuffix))
                .texture("side", this.texture(this.name(block) + "_side"))
                .texture("end", this.texture(this.name(block) + "_end"));
    }
    ...
    public void pane(IronBarsBlock block, HalfTransparentBlock glass, String location) {
        this.paneBlockWithRenderType(block, this.texture(this.name(glass), location), this.extend(this.texture(this.name(block), location), "_top"), new ResourceLocation("translucent"));
    }
    ...
    public void moaEgg(Block block) {
        ModelFile moaEgg = this.models().withExistingParent(this.name(block), this.mcLoc("block/block"))
                .texture("particle", this.texture(this.name(AetherIIBlocks.WOVEN_SKYROOT_STICKS.get())));
    ...
    public void skyrootCraftingTable(Block block, Block baseBlock, String location) {
        ResourceLocation baseTexture = this.texture(this.name(baseBlock));
    ...
    public void altar(Block block) {
        this.getVariantBuilder(block).forAllStatesExcept((state) -> {
            ...
            ModelFile model = this.models().withExistingParent(this.name(block), this.modLoc("block/template_altar"))
                    .texture("base_bottom", this.extend(this.texture(this.name(block)), "_base_bottom"))
                    .texture("base_top", this.extend(this.texture(this.name(block)), "_base_top"))
                    .texture("bottom", this.extend(this.texture(this.name(block)), "_bottom"))
                    .texture("side", this.extend(this.texture(this.name(block)), "_side"))
                    .texture("top", this.extend(this.texture(this.name(block)), "_top"))
                    .renderType("cutout");
    ...
    public void artisansBench(Block block) {
        ModelFile model = models().withExistingParent(name(block), modLoc("block/template_artisans_bench"))
                .texture("down", this.extend(this.texture(this.name(block)), "_bottom"))
                .texture("east", this.extend(this.texture(this.name(block)), "_right"))
                .texture("inside", this.extend(this.texture(this.name(block)), "_inside"))
                .texture("north", this.extend(this.texture(this.name(block)), "_front"))
                .texture("particle", this.texture(this.name(AetherIIBlocks.HOLYSTONE_BRICKS.get())))
                .texture("saw", this.extend(this.texture(this.name(block)), "_saw"))
                .texture("south", this.extend(this.texture(this.name(block)), "_back"))
                .texture("up", this.extend(this.texture(this.name(block)), "_top"))
                .texture("west", this.extend(this.texture(this.name(block)), "_left"))
                .renderType("cutout");
        this.getVariantBuilder(block).forAllStatesExcept((state) -> {
    ...
    public void decorativePillar(FacingPillarBlock block, Block endBlock) {
        ResourceLocation side = this.texture(this.name(block), "decorative/");
        ResourceLocation end = this.texture(this.name(endBlock), "decorative/");
    ...
    public void skyrootChest(Block block) {
        ModelFile chest = this.models().withExistingParent(this.name(block), this.mcLoc("block/block"))
                .texture("particle", this.texture(this.name(AetherIIBlocks.SKYROOT_PLANKS.get())));
        this.simpleBlock(block, chest);
    }
