...
public class AetherIIShroudwingVariants {
    ...
    private static ResourceKey<ShroudwingVariant> createKey(String name) {
        return ResourceKey.create(AetherIIRegistries.SHROUDWING_VARIANT, new ResourceLocation(AetherII.MODID, name));
    }

    public static void bootstrap(BootstapContext<ShroudwingVariant> context) {
    ...
    private static PriorityProvider.Selector<SpawnContext, SpawnCondition> biomes(BootstapContext<ShroudwingVariant> context, TagKey<Biome> biomeTag) {
        return new PriorityProvider.Selector<>(biomeCheck(context, biomeTag), 1);
    }

    private static BiomeCheck biomeCheck(BootstapContext<ShroudwingVariant> context, TagKey<Biome> biomeTag) {
        HolderSet<Biome> biomes = context.lookup(Registries.BIOME).getOrThrow(biomeTag);
        return new BiomeCheck(biomes);
    }

    private static void register(BootstapContext<ShroudwingVariant> context, ResourceKey<ShroudwingVariant> key, SpawnPrioritySelectors spawnSelectors) {
        register(context, key, spawnSelectors, false);
    }

    private static void register(BootstapContext<ShroudwingVariant> context, ResourceKey<ShroudwingVariant> key, PriorityProvider.Selector<SpawnContext, SpawnCondition>... spawnSelectors) {
        register(context, key, new SpawnPrioritySelectors(List.of(spawnSelectors)), false);
    }

    private static void register(BootstapContext<ShroudwingVariant> context, ResourceKey<ShroudwingVariant> key, SpawnPrioritySelectors spawnConditions, boolean emissive) {
        ResourceLocation texture = new ResourceLocation(AetherII.MODID, "textures/entity/mobs/shroudwing/" + key.location().getPath() + ".png");
        ResourceLocation emissiveTexture = new ResourceLocation(AetherII.MODID, "textures/entity/mobs/shroudwing/" + key.location().getPath() + "_glow.png");
