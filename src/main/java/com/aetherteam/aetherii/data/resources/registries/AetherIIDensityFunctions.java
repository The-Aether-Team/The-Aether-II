...
public class AetherIIDensityFunctions extends AetherIIDensityFunctionBuilders {
    ...
    public static void bootstrap(BootstapContext<DensityFunction> context) {
        ...
        context.register(CLOUDBED_NOISE, DensityFunctions.mul(new PerlinNoiseFunction(new NormalNoise.NoiseParameters(0, 1, 1, 1, 1, 1), 0.015D, 0.0D, 42), DensityFunctions.constant(2D)));
        context.register(CLOUDBED_Y_OFFSET, DensityFunctions.mul(new PerlinNoiseFunction(new NormalNoise.NoiseParameters(0, 1, 1), 0.003D, 0.0D, 95), DensityFunctions.constant(2D)));
    }
