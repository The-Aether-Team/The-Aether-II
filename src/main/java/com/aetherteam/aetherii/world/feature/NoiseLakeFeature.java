...
public class NoiseLakeFeature extends Feature<NoiseLakeConfiguration> {
            ...
            if (floor < floorNoiseValue) {
                for (int i = 0; i < barrier; i++) {
                    if (!level.isEmptyBlock(pos)
                            ...
                            && (!level.getBlockState(pos.above()).isSolid() || level.getBlockState(pos.above()).is(AetherIIBlocks.ARCTIC_ICE.get()) || level.getBlockState(pos.above()).is(AetherIIBlocks.FRAGILE_ARCTIC_ICE.get()))
                    ) {
    ...
    public void placeShore(FeaturePlaceContext<NoiseLakeConfiguration> context, BlockPos pos) {
        ...
        if (density > config.shoreStartValue() + shore - 0.005) {
            if (level.getBlockState(pos.above()).is(AetherIIBlocks.AETHER_GRASS_BLOCK.get())) {
            ...
            if (level.getBlockState(pos.above(2)).is(AetherIIBlocks.AETHER_GRASS_BLOCK.get())) {
                this.setBlock(level, pos.below(), AetherIIBlocks.AETHER_DIRT.get().defaultBlockState());
