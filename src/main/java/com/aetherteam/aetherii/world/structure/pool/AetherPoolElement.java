...
public class AetherPoolElement extends StructurePoolElement {
    ...
    public static final Codec<AetherPoolElement> CODEC = RecordCodecBuilder.create(
            instance -> instance.group(
                    ...
                    processorsCodec(),
                    projectionCodec(),
                    Codec.STRING.optionalFieldOf("override_liquid_settings").forGetter(structure -> structure.overrideLiquidSettings),
                    Codec.INT.optionalFieldOf("discard_below_y", 96).forGetter(structure -> structure.discardBelowY),
                    Codec.INT.optionalFieldOf("discard_above_y", 384).forGetter(structure -> structure.discardAboveY),
                    Codec.BOOL.optionalFieldOf("replace_air", true).forGetter(structure -> structure.replaceAir)
            ).apply(instance, AetherPoolElement::new)
    );

    protected final Either<ResourceLocation, StructureTemplate> template;
    protected final Holder<StructureProcessorList> processors;
    protected final Optional<String> overrideLiquidSettings;
    ...
    public AetherPoolElement(Either<ResourceLocation, StructureTemplate> template, Holder<StructureProcessorList> processors, StructureTemplatePool.Projection projection, Optional<String> overrideLiquidSettings, int discardBelowY, int discardAboveY, boolean replaceAir) {
        super(projection);
    ...
    public AetherPoolElement(Either<ResourceLocation, StructureTemplate> template, Holder<StructureProcessorList> processors, StructureTemplatePool.Projection projection, int discardBelowY, int discardAboveY, boolean replaceAir) {
        this(template, processors, projection, Optional.empty(), discardBelowY, discardAboveY, replaceAir);
    }

    @Override
    public Vec3i getSize(StructureTemplateManager templateManager, Rotation rotation) {
        return this.getTemplate(templateManager).getSize(rotation);
    }
    ...
    public List<StructureTemplate.StructureBlockInfo> getDataMarkers(StructureTemplateManager templateManager, BlockPos pos, Rotation rotation, boolean relativePosition) {
        ...
        List<StructureTemplate.StructureBlockInfo> filtered = structureTemplate.filterBlocks(
                pos, new StructurePlaceSettings().setRotation(rotation), Blocks.STRUCTURE_BLOCK, relativePosition
        );
        List<StructureTemplate.StructureBlockInfo> markers = Lists.newArrayList();

        for (StructureTemplate.StructureBlockInfo structureBlockInfo : filtered) {
            CompoundTag tag = structureBlockInfo.nbt();
            if (tag != null) {
                StructureMode mode = StructureMode.valueOf(tag.getString("mode"));
                if (mode == StructureMode.DATA) {
                    markers.add(structureBlockInfo);
                }
            }
        }
    ...
    public BoundingBox getBoundingBox(StructureTemplateManager templateManager, BlockPos pos, Rotation rotation) {
        return this.getTemplate(templateManager).getBoundingBox(new StructurePlaceSettings().setRotation(rotation), pos);
    }
    ...
    public boolean place(StructureTemplateManager templateManager, WorldGenLevel level, StructureManager structureManager, ChunkGenerator generator, BlockPos offset, BlockPos pos, Rotation rotation, BoundingBox box, RandomSource random, boolean keepJigsaws) {
        ...
        if (offset.getY() > this.discardBelowY && offset.getY() < this.discardAboveY) {
    ...
    protected StructurePlaceSettings getSettings(Rotation rotation, BoundingBox boundingBox, boolean offset) {
        ...
        this.overrideLiquidSettings.ifPresent(liquidSettings -> settings.setKeepLiquids(!liquidSettings.equals("ignore_waterlogging")));
