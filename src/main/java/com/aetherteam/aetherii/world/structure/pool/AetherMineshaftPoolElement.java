 ...
 */
public class AetherMineshaftPoolElement extends StructurePoolElement {
    ...
    private static final Codec<Either<Identifier, StructureTemplate>> TEMPLATE_CODEC = Codec.of(
            AetherMineshaftPoolElement::encodeTemplate, Identifier.CODEC.map(Either::left)
    );
    public static final MapCodec<AetherMineshaftPoolElement> CODEC = RecordCodecBuilder.mapCodec(
            ...
            ).apply(instance, AetherMineshaftPoolElement::new)
    );
    ...
    protected static <E extends AetherMineshaftPoolElement> RecordCodecBuilder<E, Holder<StructureProcessorList>> processorsCodec() {
        return StructureProcessorType.LIST_CODEC.fieldOf("processors").forGetter(codec -> codec.processors);
    }

    protected static <E extends AetherMineshaftPoolElement> RecordCodecBuilder<E, Optional<LiquidSettings>> overrideLiquidSettingsCodec() {
        return LiquidSettings.CODEC.optionalFieldOf("override_liquid_settings").forGetter((codec) -> codec.overrideLiquidSettings);
    }

    protected static <E extends AetherMineshaftPoolElement> RecordCodecBuilder<E, Either<Identifier, StructureTemplate>> templateCodec() {
        return TEMPLATE_CODEC.fieldOf("location").forGetter(codec -> codec.template);
    }

    public AetherMineshaftPoolElement(Either<Identifier, StructureTemplate> template, Holder<StructureProcessorList> processors, StructureTemplatePool.Projection projection, Optional<LiquidSettings> overrideLiquidSettings, int discardBelowY, int discardAboveY, boolean replaceAir) {
        super(projection);
    ...
    public boolean place(StructureTemplateManager templateManager, WorldGenLevel level, StructureManager structureManager, ChunkGenerator generator, BlockPos offset, BlockPos pos, Rotation rotation, BoundingBox box, RandomSource random, LiquidSettings liquidSettings, boolean keepJigsaws) {
        ...
        if (offset.getY() > this.discardBelowY && offset.getY() < this.discardAboveY && level.getBlockState(box.getCenter().atY(box.minY())).is(Blocks.AIR)) { // Discards a template above/below a certain y level
    ...
    public StructurePoolElementType<?> getType() {
        return AetherIIPoolElementTypes.AETHER_MINESHAFT.get();
    }
