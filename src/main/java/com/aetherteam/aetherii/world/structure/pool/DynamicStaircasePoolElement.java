...
public class DynamicStaircasePoolElement extends StructurePoolElement {
    ...
    private static final Codec<Either<ResourceLocation, StructureTemplate>> TEMPLATE_CODEC = Codec.of(
            DynamicStaircasePoolElement::encodeTemplate, ResourceLocation.CODEC.map(Either::left)
    );
    public static final MapCodec<DynamicStaircasePoolElement> CODEC = RecordCodecBuilder.mapCodec(
    ...
    protected static <E extends DynamicStaircasePoolElement> RecordCodecBuilder<E, Optional<LiquidSettings>> overrideLiquidSettingsCodec() {
        return LiquidSettings.CODEC.optionalFieldOf("override_liquid_settings").forGetter((codec) -> codec.overrideLiquidSettings);
    }

    protected static <E extends DynamicStaircasePoolElement> RecordCodecBuilder<E, Either<ResourceLocation, StructureTemplate>> templateCodec() {
        return TEMPLATE_CODEC.fieldOf("location").forGetter(codec -> codec.template);
    }

    protected static <E extends DynamicStaircasePoolElement> RecordCodecBuilder<E, Either<ResourceLocation, StructureTemplate>> templateOtherCodec() {
    ...
    public boolean place(StructureTemplateManager templateManager, WorldGenLevel level, StructureManager structureManager, ChunkGenerator generator, BlockPos offset, BlockPos pos, Rotation rotation, BoundingBox box, RandomSource random, LiquidSettings liquidSettings, boolean keepJigsaws) {
        StructureTemplate template = this.getTemplate(templateManager, offset);
    ...
    public StructurePoolElementType<?> getType() {
        return AetherIIPoolElementTypes.DYNAMIC_STAIRCASE.get();
    }
