...
import com.aetherteam.aetherii.world.structure.piece.guardiantree.GuardianTreeStructureCover;
 ...
 */
public class GuardianTreeEntrancePoolElement extends StructurePoolElement {
    ...
    private static final Codec<Either<Identifier, StructureTemplate>> TEMPLATE_CODEC = Codec.of(
            GuardianTreeEntrancePoolElement::encodeTemplate, Identifier.CODEC.map(Either::left)
    );
    public static final MapCodec<GuardianTreeEntrancePoolElement> CODEC = RecordCodecBuilder.mapCodec(
            ...
            ).apply(instance, GuardianTreeEntrancePoolElement::new)
    );
    ...
    protected static <E extends GuardianTreeEntrancePoolElement> RecordCodecBuilder<E, Holder<StructureProcessorList>> processorsCodec() {
        return StructureProcessorType.LIST_CODEC.fieldOf("processors").forGetter(codec -> codec.processors);
    }

    protected static <E extends GuardianTreeEntrancePoolElement> RecordCodecBuilder<E, Optional<LiquidSettings>> overrideLiquidSettingsCodec() {
        return LiquidSettings.CODEC.optionalFieldOf("override_liquid_settings").forGetter((codec) -> codec.overrideLiquidSettings);
    }

    protected static <E extends GuardianTreeEntrancePoolElement> RecordCodecBuilder<E, Either<Identifier, StructureTemplate>> templateCodec() {
        return TEMPLATE_CODEC.fieldOf("location").forGetter(codec -> codec.template);
    }

    public GuardianTreeEntrancePoolElement(Either<Identifier, StructureTemplate> template, Holder<StructureProcessorList> processors, StructureTemplatePool.Projection projection, Optional<LiquidSettings> overrideLiquidSettings, int discardBelowY, int discardAboveY, boolean replaceAir) {
        super(projection);
        ...
        if (offset.getY() > this.discardBelowY && offset.getY() < this.discardAboveY) { // Discards a template above/below a certain y level
            if (!template.placeInWorld(level, offset, pos, settings, random, 18)) {
                ...
                for (StructureTemplate.StructureBlockInfo structureBlockInfo : StructureTemplate.processBlockInfos(
                    ...
                    //new GuardianTreeStructureCover(pos);
    ...
    public StructurePoolElementType<?> getType() {
        return AetherIIPoolElementTypes.ENTRANCE.get();
    }
