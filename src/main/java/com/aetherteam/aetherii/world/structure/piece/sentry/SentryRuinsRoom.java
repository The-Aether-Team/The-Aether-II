...
public class SentryRuinsRoom extends SentryRuinsPiece {
    ...
    protected void handleDataMarker(String name, BlockPos pos, ServerLevelAccessor level, RandomSource random, BoundingBox box) {
        ...
        if (name.equals("Sentry Golem")) {
            ...
            sentryGolem.setItemSlot(EquipmentSlot.MAINHAND, new ItemStack(AetherIIItems.HAMMER_OF_DEMOLITION.get()));
            ...
            if (name.equals("Entrance Moss")) {
                if (level.getBiome(pos).is(AetherIITags.Biomes.ARCTIC)) {
                    this.placeConfiguredFeature(level, random, pos, HolyIslesConfiguredFeatures.SHAYELINN_MOSS_STRUCTURE);
                } else if (level.getBiome(pos).is(AetherIITags.Biomes.IRRADIATED)) {
                    this.placeConfiguredFeature(level, random, pos, HolyIslesConfiguredFeatures.AMBRELINN_MOSS_STRUCTURE);
                }
                else this.placeConfiguredFeature(level, random, pos, HolyIslesConfiguredFeatures.BRYALINN_MOSS_STRUCTURE);
            } else {
                this.placeConfiguredFeature(level, random, pos, ResourceKey.create(Registries.CONFIGURED_FEATURE, new ResourceLocation(name)));
            }
        }
    }

    private void placeConfiguredFeature(ServerLevelAccessor level, RandomSource random, BlockPos pos, ResourceKey<ConfiguredFeature<?, ?>> key) {
        ConfiguredFeature<?, ?> feature = level.registryAccess().registryOrThrow(Registries.CONFIGURED_FEATURE).getHolderOrThrow(key).value();
