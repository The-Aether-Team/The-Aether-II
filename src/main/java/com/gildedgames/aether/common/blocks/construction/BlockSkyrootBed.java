...
public class BlockSkyrootBed extends BlockHorizontal
{
	public BlockSkyrootBed()
	{
		super(Material.CLOTH);
		this.setDefaultState(this.blockState.getBaseState().withProperty(PART, BlockSkyrootBed.EnumPartType.FOOT).withProperty(OCCUPIED, Boolean.valueOf(false)));
	}

	public static final PropertyEnum<BlockSkyrootBed.EnumPartType> PART = PropertyEnum.<BlockSkyrootBed.EnumPartType>create("part", BlockSkyrootBed.EnumPartType.class);
	...
	public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, @Nullable ItemStack heldItem, EnumFacing side, float hitX, float hitY, float hitZ)
	{
		...
		{
			if (state.getValue(PART) != BlockSkyrootBed.EnumPartType.HEAD)
			{
	...
	{
		return state.getValue(PART) == BlockSkyrootBed.EnumPartType.HEAD ? null : ItemsAether.skyroot_bed;
	}
	...
	{
		if (state.getValue(PART) == BlockSkyrootBed.EnumPartType.FOOT)
		{
	...
	{
		return new ItemStack(ItemsAether.skyroot_bed);
	}

	public void onBlockHarvested(World worldIn, BlockPos pos, IBlockState state, EntityPlayer player)
	{
		if (player.capabilities.isCreativeMode && state.getValue(PART) == BlockSkyrootBed.EnumPartType.HEAD)
		{
			BlockPos blockpos = pos.offset(((EnumFacing)state.getValue(FACING)).getOpposite());
	...
	public IBlockState getStateFromMeta(int meta)
	{
		...
		return (meta & 8) > 0 ? this.getDefaultState().withProperty(PART, BlockSkyrootBed.EnumPartType.HEAD).withProperty(FACING, enumfacing).withProperty(OCCUPIED, Boolean.valueOf((meta & 4) > 0)) : this.getDefaultState().withProperty(PART, BlockSkyrootBed.EnumPartType.FOOT).withProperty(FACING, enumfacing);
	}
