...
public class EntityBolt extends Entity implements IProjectile
{
	...
	public EntityBolt(World world, EntityLivingBase shooter, EntityLivingBase prey, float fromHeight, float damage)
	{
		super(world);
	...
	public EntityBolt(World worldIn, EntityLivingBase shooter, float velocity)
	{
		this(worldIn);
	...
	public void onUpdate()
	{
		...
		ItemBoltType boltType = this.getBoltType();
	...
	{
		if (!this.worldObj.isRemote && this.inGround && this.boltShake <= 0)
		{
			boolean canPickup = this.canPickup == 1 || this.canPickup == 2 && player.capabilities.isCreativeMode;

			if (this.canPickup == 1 && !player.inventory.addItemStackToInventory(new ItemStack(ItemsAether.bolt, 1, this.getBoltType().getAmmoItem().ordinal())))
			{
				canPickup = false;
			}
	...
	public int getBoltShake()
	{
		return this.boltShake;
	}
