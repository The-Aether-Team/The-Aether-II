...
public class CurrencyAether
{
	...
	public static void serverStarted()
	{
		...
		c.registerValue(new ItemStack(BlocksAether.blue_skyroot_leaves, 1, OreDictionary.WILDCARD_VALUE), builder.gilt(1).flush());
		...
		c.registerValue(new ItemStack(BlocksAether.dark_blue_skyroot_leaves, 1, OreDictionary.WILDCARD_VALUE), builder.gilt(1).flush());
		...
		{
			registerBlockCurrency(block, builder, c);
		}

		for (Item item : ItemsAether.getRegisteredItems())
		{
			registerItemCurrency(item, builder, c);
		}

		for (ISimpleRecipe recipe : AetherAPI.content().masonry().getAllRecipes())
		{
			if (recipe.getResult() != null && !c.hasValue(recipe.getResult()))
			{
				double value = 0;
				...
				c.registerValue(recipe.getResult(), value);
			}
		}

		c.registerValue(new ItemStack(ItemsAether.secret_skyroot_door, 1, OreDictionary.WILDCARD_VALUE),
				c.getValue(new ItemStack(ItemsAether.skyroot_door, 1, OreDictionary.WILDCARD_VALUE)));
		c.registerValue(new ItemStack(BlocksAether.secret_skyroot_trapdoor, 1, OreDictionary.WILDCARD_VALUE),
				c.getValue(new ItemStack(BlocksAether.skyroot_trapdoor, 1, OreDictionary.WILDCARD_VALUE)));
	}

	private static void registerBlockCurrency(Block block, GiltBuilder builder, ICurrencyRegistry c)
	{
		...
		for (IBlockState state : block.getBlockState().getValidStates())
		{
			...
			registerCurrency(null, stack, builder, c);
		}
	...
	private static void registerItemCurrency(Item item, GiltBuilder builder, ICurrencyRegistry c)
	{
	...
	private static void registerCurrency(ItemStack recursiveParent, ItemStack stack, GiltBuilder builder, ICurrencyRegistry c)
	{
		...
		for (IIndexableRecipe recipe : recipes)
		{
			...
			for (Ingredient ingredient : recipe.getCraftingMatrix())
			{
				...
				{
					if (!c.hasValue(s) && (recursiveParent == null || ItemHelper.getHashForItemStack(recursiveParent) != ItemHelper.getHashForItemStack(s)))
					{
						if (recursiveParent == null)
						{
							registerCurrency(s, s, new GiltBuilder(), c);
						}
						else
						{
							registerCurrency(recursiveParent, s, new GiltBuilder(), c);
						}
					}
		...
		if (highestValue > 0)
		{
			c.registerValue(stack, highestValue);
		}
