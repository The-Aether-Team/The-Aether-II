...
public class WorldGenFruitTree extends WorldGenAbstractTree
{
	...
	public WorldGenFruitTree(final IBlockState leaves, final IBlockState fruit, final int fruitChance, final int heightWeight, final boolean branchFlag)
	{
		super(false);
	...
	public void setBlockAirCheck(final World world, final int x, final int y, final int z, final IBlockState block)
	{
	...
	public void setBlockAirCheck(final World world, final int x, final int y, final int z, final IBlockState log, final IBlockState leaves)
	{
		final BlockPos pos = new BlockPos(x, y, z);
	...
	public boolean generate(final World world, final Random random, final BlockPos position)
	{
		...
		if (y + (height + 2) <= world.getActualHeight() && cangen)
		{
			...
			final Block y1 = world.getBlockState(pos).getBlock();
			...
			final IBlockState wall = BlocksAether.skyroot_log_wall.getDefaultState().withProperty(BlockSkyrootWall.PROPERTY_GENERATED, true);
			final IBlockState log = BlocksAether.skyroot_log.getDefaultState();
			...
			if (this.branches)
			{
				...
				{
					final int chance = Math.max(1, (Math.max(1, y * (y2 - y)) / y2));
